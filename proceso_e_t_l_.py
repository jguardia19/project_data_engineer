...
def insert_into_hive(clean_data, debug):
    ...
    conn = hive.Connection(host=hiveqlhost, port=hiveipport,
                           username=hiveuser, database=hivedatabase, auth=hiveauth)
    cursor = conn.cursor()
    ...
    for i, start in enumerate(range(0, n, batch_size), start=1):
        ...
        print(
            f"Enviando paquete {i} de {total_batches}... ({len(chunk)} registros)")
...
def process_csv_and_insert_into_hive(debug):
    ...
    try:
        ...
        csvsentpath = os.path.join(
            script_dir, "eventosdetectadosnvidia_inHiveQL.csv")
        ...
        if not bSentFile and not bNvidiaFile:
            ...
            clean_data = nvidia_without_duplicates[~nvidia_without_duplicates.isin(
                df_sent).all(axis=1)]
...
def main(debug):
    ...
    schedule.every(10).seconds.do(
        lambda: process_csv_and_insert_into_hive(debug))
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description="Arguments to setup the Camera Object")

    parser.add_argument('--debug', dest='debug',
                        action='store_true', help="To do debug")
    parser.add_argument('--no-debug', dest='debug',
                        action='store_false', help="To do debug")
